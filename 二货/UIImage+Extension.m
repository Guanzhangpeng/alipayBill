...
+ (UIImage *)circleImageWithName:(NSString *)imgName borderWidth:(CGFloat)border borderColor:(UIColor *)color{
    UIImage *oldImg = [UIImage imageNamed:imgName];
    // 1.开启新的上下文
    CGSize size = CGSizeMake(oldImg.size.width + border * 2, oldImg.size.height + border * 2);
    ...
    CGRect bigCircleRect = CGRectMake(0, 0, oldImg.size.width + border * 2, oldImg.size.height + border * 2);
    CGContextAddEllipseInRect(ctx, bigCircleRect);
    [color set];
    CGContextFillPath(ctx);
    // 画一个小圆，并裁减
    CGRect smallCircleRect = CGRectMake(border, border, oldImg.size.width, oldImg.size.height);
    CGContextAddEllipseInRect(ctx, smallCircleRect);
    CGContextClip(ctx);
    // 4.画图片
    [oldImg drawInRect:smallCircleRect];
    // 5.取出合成后的图片
